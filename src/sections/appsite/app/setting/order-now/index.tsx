...
export const Schema = (maxQuantity: number, value: string) => Yup.object().shape({
  address: value === 'currentAddess' ? Yup.string().notRequired() : Yup.string().required('Address is required'),
  quantity: Yup.number()
    ...
    >
      <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={2} padding={2}>
          <Typography variant="body2" fontWeight={'bold'}>Select Order for Address </Typography>
          <Grid item xs={12} >
            <RHFRadioGroup
              name="addressValue"
              row={false}
          ...
          {addressValues === 'addNewAddress' && (
            <>
              <Grid item xs={6} >
                <RHFTextField name="address" size="small" outerLabel="Add New Address" />
              </Grid>
          ...
          <Grid item xs={6} >
            <RHFTextField name="quantity" size="small" outerLabel="Add Card Quantity" type='number' />
          </Grid>
          <Grid item xs={6} />
          <Grid item xs={6}>
            <LoadingButton variant="contained" type="submit" loading={isLoading}>Confirm Address</LoadingButton>
          </Grid>
        </Grid>
      </FormProvider>
    </CustomModal>
